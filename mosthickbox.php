...
$mainframe->registerEvent( 'onPrepareContent', 'mosthickbox' );
$html = "";

$plugin =& JPluginHelper::getPlugin('content', 'mosthickbox');

if (JPluginHelper::isEnabled('content', 'mosthickbox'))
{
	...
	if ( !($int = $pluginParams->get( 'slimbox' )))
	{
		$pluginParams->set( 'slimbox', 0 );
		$int = $pluginParams->get( 'slimbox');
	}

	$th_width = $pluginParams->get( 'thumbnail_width' );
	$th_height = $pluginParams->get( 'thumbnail_height' );
	$th_quality = $pluginParams->get( 'thumbnail_quality' );
...
function mosthickbox ( &$row , &$params )
{
	//$published = $row->state > 0; // Forum Tip...
	if (JPluginHelper::isEnabled('content', 'mosthickbox'))
	{
		$row->text = procBox($row->text);
		return true;
	} else {
				...
				while (($f = readdir($dir)) !== false) {
					...
					if(in_array($ext,array('jpg','gif','png'))) {
						...
						if (!file_exists($thumb)){
							// generate thumb
							makeThumb($imgdir.'/'.$f,$ext, $th_width, $th_height, $th_quality);
...
function makeThumb($_image_, $ext, $_width_min_, $_height_min_, $_quality_){
